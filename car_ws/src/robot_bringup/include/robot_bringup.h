...
#ifndef __ROBOT_BRINGUP_H_
#define __ROBOT_BRINGUP_H_
...
// 抓取命令帧格式（4字节）：0x6B | 状态(0x00/0x01) | BCC | 0x6D
#define GRAB_FRAME_HEADER 0x6B
#define GRAB_FRAME_TAIL   0x6D
...
class turn_on_robot
{
	public:
		...
		serial::Serial Stm32_Serial; //Declare a serial object //声明STM32串口对象
		serial::Serial Arduino_Serial; //Declare Arduino serial object //声明Arduino串口对象 
	private:
		...
		ros::Subscriber Grab_State_Sub; //订阅抓取状态 //订阅抓取状态
		//The speed topic subscribes to the callback function
		//速度话题订阅回调函数
		void Cmd_Vel_Callback(const geometry_msgs::Twist &twist_aux);
		// 抓取状态订阅回调函数：向STM32发送4字节抓取命令
		void Grap_state_Callback(const std_msgs::Int32 &grab_aux);
        ...
        string usart_port_name, arduino_port_name, robot_frame_id, gyro_frame_id, odom_frame_id; //Define the related variables //定义相关变量
        int serial_baud_rate, arduino_baud_rate;      //Serial communication baud rate //串口通信波特率
