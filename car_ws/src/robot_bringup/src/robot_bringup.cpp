#include "../include/robot_bringup.h"
#include "../include/Quaternion_Solution.h"
...
{
  ros::init(argc, argv, "robot_bringup"); //ROS initializes and sets the node name //ROS初始化 并设置节点名称 
...
/**************************************
Date: 2025-08-09
Function: Grab state topic subscription Callback function, send 4-byte frame to Arduino
功能: 抓取状态订阅回调函数，向Arduino发送4字节帧：0x6B | 状态(0x00/0x01) | BCC | 0x6D
***************************************/
void turn_on_robot::Grap_state_Callback(const std_msgs::Int32 &grab_aux)
{
  uint8_t frame[4] = {0};
  frame[0] = GRAB_FRAME_HEADER; // 帧头 0x6B
  // 第二个字节：抓取状态，来自 /grab 话题，数值 0 或 1，但按 16 进制发送
  frame[1] = (grab_aux.data == 0) ? 0x00 : 0x01;
  // 第三个字节：BCC 校验（对前两个字节按位异或）
  uint8_t bcc = frame[0] ^ frame[1];
  frame[2] = bcc;
  // 第四个字节：帧尾 0x6D
  frame[3] = GRAB_FRAME_TAIL;

  try
  {
    Arduino_Serial.write(frame, sizeof(frame));
  }
  catch (serial::IOException& e)
  {
    ROS_ERROR_STREAM("Unable to send grab frame through Arduino serial port");
  }
}
...
turn_on_robot::turn_on_robot():Sampling_Time(0),Power_voltage(0)
{
  ...
  private_nh.param<std::string>("usart_port_name",  usart_port_name,  "/dev/stm32-device"); //Fixed serial port number //固定串口号
  ...
  private_nh.param<std::string>("arduino_port_name", arduino_port_name, "/dev/arduino-device"); //Arduino serial port number //Arduino串口号
  private_nh.param<int>        ("arduino_baud_rate", arduino_baud_rate, 115200); //Arduino communication baud rate //Arduino通信波特率
  ...
  //Set the grab state subscription callback function
  //抓取状态订阅回调函数设置
  Grab_State_Sub = n.subscribe("grab", 100, &turn_on_robot::Grap_state_Callback, this);
  ...
  // Initialize Arduino serial port //初始化Arduino串口
  try
  { 
    //Attempts to initialize and open the Arduino serial port //尝试初始化与开启Arduino串口
    Arduino_Serial.setPort(arduino_port_name); //Select the Arduino serial port number to enable //选择要开启的Arduino串口号
    Arduino_Serial.setBaudrate(arduino_baud_rate); //Set the Arduino baud rate //设置Arduino波特率
    serial::Timeout arduino_time = serial::Timeout::simpleTimeout(2000); //Timeout //超时等待
    Arduino_Serial.setTimeout(arduino_time);
    Arduino_Serial.open(); //Open the Arduino serial port //开启Arduino串口
  }
  catch (serial::IOException& e)
  {
    ROS_ERROR_STREAM("Arduino can not open serial port, Please check the Arduino serial port cable! "); //If opening the Arduino serial port fails, an error message is printed //如果开启Arduino串口失败，打印错误信息
  }
  if(Arduino_Serial.isOpen())
  {
    ROS_INFO_STREAM("Arduino serial port opened"); //Arduino serial port opened successfully //Arduino串口开启成功提示
  }
}
/**************************************
Date: January 28, 2021
...
turn_on_robot::~turn_on_robot()
{
  ...
  Stm32_Serial.close(); //Close the STM32 serial port //关闭STM32串口
  
  // Close Arduino serial port //关闭Arduino串口
  if(Arduino_Serial.isOpen())
  {
    Arduino_Serial.close(); //Close the Arduino serial port //关闭Arduino串口
  }
